...
export const mapUserDto = (dto: any) => ({
  ...
  companyId: (dto.muessiseId || dto.MuessiseId || dto.companyId)?.toString() || undefined,
...
// TASK STATUS MAPPER
const mapTaskItemStatus = (s: string): 'gozlenir' | 'icrada' | 'tamamlandi' | undefined => {
...
export const mapTaskDto = (dto: any) => ({
  ...
  deadline: (dto.Deadline || dto.deadline)
  ...
  tamamlanmaTarixi: (dto.CompletedAt || dto.completedAt)
...
export const mapAnnouncementDto = (dto: any) => ({
  ...
  yaranmaTarixi: (dto.CreatedAt || dto.createdAt)
    ? new Date(dto.CreatedAt || dto.createdAt).toLocaleString('az-AZ')
    : '',
  oxuyanlar: dto.ReadByLogins || dto.readByLogins || [],
  alicilar: (dto.IsForAll || dto.isForAll) ? 'hamisi' as const : (dto.Recipients || dto.recipients || []),
...
export const authAPI = {
  login: async (login: string, parol: string) =>
    request('/auth/login', { method: 'POST', body: JSON.stringify({ username: login, password: parol }) }),
...
export const tasksAPI = {
  ...
  create: async (task: any) => {
    const dto = {
      ...
      AssigneeIds: (task.secilmisShexsler || []).map((s: any) => s.id).filter(Boolean),
      Deadline: task.deadline ? new Date(task.deadline + 'T00:00:00').toISOString() : null,
    ...
    const result = await request('/tasks', { method: 'POST', body: JSON.stringify(dto) })
  ...
  update: async (id: string, data: any) => {
    ...
    const result = await request(`/tasks/${id}`, { method: 'PUT', body: JSON.stringify(dto) })
  ...
  addComment: async (id: string, text: string) =>
    request(`/tasks/${id}/comments`, { method: 'POST', body: JSON.stringify({ Text: text }) }),
}
...
export const elanlarAPI = {
  ...
  create: async (elan: any) => {
    ...
    const result = await request('/announcements', { method: 'POST', body: JSON.stringify(dto) })
...
export const mapMuessiseDto = (dto: any) => ({
  ...
  yaranmaTarixi: (dto.YaranmaTarixi || dto.yaranmaTarixi)
    ? new Date(dto.YaranmaTarixi || dto.yaranmaTarixi).toLocaleDateString('az-AZ')
    : '',
...
export const notesAPI = {
  ...
  create: async (_login: string, note: any) => {
    ...
    const result = await request('/notes', { method: 'POST', body: JSON.stringify(dto) })
  ...
  update: async (_login: string, id: string, data: any) => {
    ...
    const result = await request(`/notes/${id}`, { method: 'PUT', body: JSON.stringify(dto) })
