...
function TaskSidePanel({ task, currentUser, onClose, onUpdateTask, onDeleteTask }: TaskSidePanelProps) {
  ...
  const handleIcraFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleIcraTesdiq = () => {
    ...
    const statusMetn = yeniStatus === 'icrada'
    ...
    onUpdateTask({ ...task, secilmisShexsler: updatedShexsler, fayllar: updatedFayllar, mesajlar: [...(task.mesajlar || []), statusMsg] });
    setIcraModal(false);
    setIcraFayl(null);
  };
  ...
  const handleCancelEdit = () => { setEditingMsgId(null); setMesaj(""); };
  ...
  const handleDelete = () => {
    if (window.confirm("Bu tapşırığı silmək istəyirsiniz?")) { onDeleteTask(task.id); onClose(); }
  };
              ...
              {m.fayl ? (
                ...
                <div className="tsp-msg-metn-row">
                  ...
                  {m.yazanLogin === currentUser.login && (
                    <button className="tsp-msg-edit-btn" onClick={() => handleStartEdit(m)} title="Redaktə et">
                      <FaPencilAlt />
                    </button>
                  )}
                </div>
      ...
      <div className="tsp-icra-modal-overlay" onClick={() => setIcraModal(false)}>
        <div className="tsp-icra-modal" onClick={(e) => e.stopPropagation()}>
          <div className="tsp-icra-modal-header">
            {isIcrada ? <FaCheckDouble className="tsp-icra-modal-icon tamamla" /> : <FaPlayCircle className="tsp-icra-modal-icon icra" />}
            <h3>{isIcrada ? "Tapşırığı tamamla" : "İcraya götür"}</h3>
          </div>
          <p className="tsp-icra-modal-metn">
            {isIcrada ? "Tapşırığı tamamladığınızı təsdiqləyirsiniz?" : "Tapşırığı icraya götürdüyünüzü təsdiqləyirsiniz?"}
          </p>
          {isIcrada && (
            <div className="tsp-icra-modal-fayl">
              <div className="tsp-icra-modal-fayl-label">
                <FaPaperclip /> Sənəd əlavə et <span className="tsp-icra-optional">(istəyə görə)</span>
              </div>
              {icraFayl ? (
                <div className="tsp-icra-fayl-item">
                  <span>📎 {icraFayl.name}</span>
                  <span className="tsp-icra-fayl-size">{formatFileSize(icraFayl.size)}</span>
                  <button onClick={() => setIcraFayl(null)}><FaTimes /></button>
                </div>
              ) : (
                <button className="tsp-icra-fayl-btn" onClick={() => icraFileRef.current?.click()}>
                  <FaPaperclip /> Fayl seç
                </button>
              )}
              <input ref={icraFileRef} type="file" style={{ display: "none" }} onChange={handleIcraFileSelect} />
            </div>
          )}
          <div className="tsp-icra-modal-btns">
            <button className="tsp-icra-modal-cancel" onClick={() => { setIcraModal(false); setIcraFayl(null); }}>Ləğv et</button>
            <button className={`tsp-icra-modal-confirm ${isIcrada ? 'tamamla' : 'icra'}`} onClick={handleIcraTesdiq}>
              {isIcrada ? <><FaCheckDouble /> Tamamladım</> : <><FaPlayCircle /> İcraya götürdüm</>}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="tsp-panel" onClick={(e) => e.stopPropagation()}>
          <div className="tsp-header">
            <div className="tsp-header-left">
              ...
              <h3 className="tsp-name" style={{ textDecoration: 'line-through', opacity: 0.7 }}>{task.tapsirigAdi}</h3>
            </div>
            ...
            <div className="tsp-col-left">
              <div className="tsp-col-scroll">
                  ...
                  {task.secilmisShexsler.length > 0 && (
                    <div className="tsp-icracilar">
                      {task.secilmisShexsler.map((s) => (
                        <div key={s.login} className={`tsp-icraci status-${(s as any).status || 'tamamlandi'}`}>
                ...
                {(task.altTapsiriglar || []).length > 0 && (
                  <div className="tsp-section">
                    <div className="tsp-section-header"><span>📋 Alt tapşırıqlar</span></div>
                ...
                {task.tamamlanmaTarixi && (
                  <div className="tsp-section">
                    <div className="tsp-section-header"><span>✅ Tamamlanma tarixi</span></div>
                    <span className="tsp-date-text">{task.tamamlanmaTarixi}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="tsp-header">
          <div className="tsp-header-left">
            ...
            {editingName ? (
              ...
              <h3 className="tsp-name" onClick={() => setEditingName(true)} title="Klikləyib adı dəyişin">{task.tapsirigAdi}</h3>
            )}
          </div>
          <div className="tsp-header-right">
            <span className={`tsp-star${task.tecili ? " aktiv" : ""}`} onClick={handleTeciliToggle}>{task.tecili ? "★" : "☆"}</span>
            <button className="tsp-close" onClick={onClose}><FaTimes /></button>
          </div>
        </div>
        <div className="tsp-columns">
            ...
            <div className="tsp-col-scroll">
              <div className="tsp-section">
                <div className="tsp-sub-add">
                  ...
                  <input type="text" className="tsp-sub-add-input" placeholder="Alt tapşırıq əlavə et..." value={yeniSubTask} onChange={(e) => setYeniSubTask(e.target.value)} onKeyDown={handleAddSubTask} />
                </div>
                {(task.altTapsiriglar || []).length > 0 && (
                  <div className="tsp-sub-list">
                    {(task.altTapsiriglar || []).map((sub) => (
              ...
              <div className="tsp-section tsp-row-section">
                ...
                <div className="tsp-row-item">
                  <div className="tsp-section-header" onClick={() => fileInputRef.current?.click()}>
                    <FaPaperclip className="tsp-section-icon" /><span>Fayl əlavə et</span>
                  </div>
                    ...
                    <div className="tsp-files">
                      {task.fayllar.map((f, i) => (
                        <div key={i} className="tsp-file-item">
                          ...
                          <span className="tsp-file-size">{formatFileSize(f.size)}</span>
                          <FaTrash className="tsp-file-del" onClick={() => handleFileRemove(i)} />
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="tsp-section tsp-user-section">
                <div className="tsp-section-header"><span>👥 İstifadəçilər</span></div>
